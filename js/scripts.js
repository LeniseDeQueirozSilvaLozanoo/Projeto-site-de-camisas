...
// Reso*ução do desafio*/:

$(function(){
// criação da função
    function atualizar_orcamento(parametros){
/* CÓDIGO PARA FAZER O LOADER DE CARREGAMENTO APARECER AO ATUALIZAR A PAGINA 
*/
       $(".refresh-loader").show();
// Código para calcular o valor da camiseta
...
// código para gerar o número de série
        parametros.nroSerie = Math.random();
// código para acrescimo pela qualidade da roupa
        if(parametros.qualidade =="q190"){
            valor_total *=1.12;
        }
        
// código para embalagem unitaria
        if(parametros.embalagem == "unitaria"){
            valor_total += (quantidade * 0.15);
        }

// código para desconto de grandes quantidades
        if(quantidade >= 1000){
            valor_total *= 0.85;        
...
$("#result_nroSerie").html(parametros.nroSerie);
